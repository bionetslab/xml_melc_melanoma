...
f = open("/data/bionets/je30bery/CTCL_analysis/config.json")
config = json.load(f)
...
for segment in ["nuclei", "cell"]:
    ...
    for fov in seg.fields_of_view:
        ...
        adata.obs = pd.DataFrame(list(exp_fov.index), columns=["cell_id"]).astype(int)

        adata.varm["antibody"] = pd.DataFrame(exp_fov.columns, columns=["antibody"])
        adata.obsm["segment_size"] = pd.DataFrame(np.array([len(where_dict[k][0]) for k in where_dict]), columns=["segment_size"])
        
        adata.uns["group"] = group
        ...
        with open(os.path.join(seg_results_path, "anndata_files", f"adata_{segment}_{fov}.pickle"), 'wb') as handle:
